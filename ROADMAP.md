...
`install.sh` should support `curl -fsSL https://…/install.sh | sh` — the web variant just fetches the wheel first. The
script is the single entry point for both "install from a built wheel" and "install from the web".
...
**Preflight.** `install.sh` checks for required tools before doing anything, and exits with a clear message if missing:
